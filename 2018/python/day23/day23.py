...
def in_range(positions):
    positions_by_range = sorted(
        [pos for pos in positions], key=lambda x: x[1], reverse=True
    )
...
def main():
    ...
    for col_name in ["x", "y", "z"]:
        print(
            "PRE: col: {} min: {}, max: {}".format(
                col_name, df[col_name].min(), df[col_name].max()
            )
        )
        df = remove_outlier(df, col_name)
        print(
            "POST: col: {} min: {}, max: {}".format(
                col_name, df[col_name].min(), df[col_name].max()
            )
        )
        print(len(df))
