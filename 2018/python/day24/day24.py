...
class Group(object):
    def __init__(self, units, dmg, hitpoints, atk_type, weaknesses, immunes, initiative, g_type, id):
        self._type = g_type
        self.units = units
        self._dmg = dmg
...
def parse(lines):
    ...
    infections = [Group(**g, g_type='Immune System', id=i+1) for i, g in enumerate(infections)]
    immune_systems = dicts_from_lines(lines[split_idx+1:])
    immune_systems = [Group(**g, g_type='Infection', id=i+1) for i, g in enumerate(immune_systems)]
    infections.extend(immune_systems)
    return infections

def selection_phase(groups):
    selection_order = sorted(groups, key = lambda x: (x.effective_power, x.initiative), reverse=True)
    ordered_attacking_groups = []
    for group in selection_order:
        already_selected = [g for _, g in ordered_attacking_groups]
        defending_groups = [g for g in groups if g.type!=group.type and
                                                 g not in already_selected]
        if defending_groups:
            possible_targets = sorted(defending_groups, 
                                      key = lambda x: (x.dmg_from_group(group), x.effective_power, x.initiative),
                                      reverse = True)
...
def attacking_phase(groups):
    ...
    for attacker, defender in groups:
        ...
        print("{} group {} attacks {} group {}, killing {} units".format(
