...
def calc_sleep_time(schedule):
    ...
    for sch in schedule:
        ...
        if guard not in sleep_times:
            sleep_times[guard] = []
        sleep_times[guard].append({'start': start,
                                   'end': end,
                                   'duration': end-start})
    return sleep_times
...
def main(data):
    ...
    print("Guard {} slept longest the minute: {}. Multiplied: {}".format(guard, minute, guard*minute))
    guard, minute, amount = minute_guard_most_freq_asleep(sleep_times)
    print("Guard {} slept the most frequent at minute {}. Multipled {}".format(guard, minute, guard*minute))
